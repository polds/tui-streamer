...
**`INTERACTIVE_COMMANDS`** is a module-level `Set` of command names (e.g. `vim`, `htop`, `tmux`, `less`, `watch`) that cannot be driven via the streaming interface. The UI shows a warning when the user attempts to run one of these commands.
